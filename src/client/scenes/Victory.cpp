...
Client::Scenes::Victory::Victory(RNGine::Core &core) {
  CreateEntity("background1");
  CreateEntity("background2");
  CreateEntity("redmoon");
  CreateEntity("greenmoon");
  core.AddSystem("on-click-button-retry-victory",
                 Client::Scenes::Victory::OnClickButtonRetry);
  core.AddSystem("on-click-button-backHome-victory",
                 Client::Scenes::Victory::OnClickButtonBackHome);
  core.AddSystem("render-Victory", Client::Scenes::Victory::renderVictory);
  CreateMouse();
  CreateButton("retry-victory", "./assets/menu/win/btn-nextlevel",
               r::GetScreenHeight() / 2);
  CreateButton("back-victory", "./assets/menu/win/btn-backtohome",
               r::GetScreenHeight() / 2 + 100);
  AddComponent(CreateEntity("numberPLayers"),
               Client::Components::NumberPlayers());
}

void Client::Scenes::Victory::CreateMouse() {
...
void Client::Scenes::Victory::CreateButton(const std::string &id,
...
void Client::Scenes::Victory::OnClickButtonRetry(RNGine::Core &core) {
  if (core.GetActualSceneName() != "victory")
    return;
  auto &scene = core.GetActualScene();
  auto &clicks = scene.GetComponents<Client::Components::IsClicked>();
  auto &click = clicks[scene.GetEntity("button-retry-victory")];
...
void Client::Scenes::Victory::OnClickButtonBackHome(RNGine::Core &core) {
  if (core.GetActualSceneName() != "victory")
    return;
  auto &scene = core.GetActualScene();
  auto &clicks = scene.GetComponents<Client::Components::IsClicked>();
  auto &click = clicks[scene.GetEntity("button-back-victory")];
...
void Client::Scenes::Victory::renderVictory(RNGine::Core &core) {
  if (core.GetActualSceneName() != "victory")
    return;
  auto &scene = core.GetActualScene();
  std::string toPrint = "Victory";
  r::DrawText(toPrint.c_str(), r::GetScreenWidth() / 2 - 325, 50, 150, r::GREEN);
}